...
import {
  DEFAULT_BARBER_CARE_TYPES_SENTENCE,
...
export async function generateMetadata({
  ...
  const title = `Barber Shops in ${stateName} | ${totalFacilities.toLocaleString()} Verified Listings | BarbershopDirectories.com`;

  const descriptor = `Browse ${totalFacilities.toLocaleString()} verified barber shops across ${cities.length.toLocaleString()} ${stateName} cities. Find barbers offering haircuts and shaves — all rated 3 stars or higher on Google Maps.`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${stateName} barber shop directory preview`,
        },
      ],
    },
  };
...
export default async function StatePage({ params }: StatePageProps) {
  ...
  const barberFocusText =
    "barber shops, barbers, haircuts, and shaves";
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: `How many barber shops are in ${stateName}?`,
        acceptedAnswer: {
          "@type": "Answer",
      ...
      {
        "@type": "Question",
        name: `What types of barber shop services are available in ${stateName}?`,
        acceptedAnswer: {
          "@type": "Answer",
      ...
      {
        "@type": "Question",
        name: "How are barber shops selected for this directory?",
        acceptedAnswer: {
          "@type": "Answer",
  ...
  const webpageSchema = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: `Barber Shops in ${stateName}`,
    ...
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <section className="rounded-2xl bg-surface-muted px-5 py-6 text-foreground shadow-lg shadow-navy/10 ring-1 ring-gold/40 sm:px-8 sm:py-8">
        ...
        <h1 className="mt-2 text-3xl font-semibold sm:text-4xl">
          Barber Shops in {stateName}
        </h1>
        <p className="mt-3 max-w-2xl text-sm text-foreground/80">
          Explore {barberFocusText} across {stateName}, including major city
          areas such as {majorCitiesText}. Use this page to find barber shops by city,
          then review{" "}
          <a
            href={resourcesUrl}
            target="_blank"
          ...
          >
            Associated Board of Cosmetology and Barber Examiners industry resources
          </a>{" "}
          for industry trends, licensing context, and how to choose a barber shop or barber.
        </p>
        ...
        return (
          <section className="mt-8 rounded-2xl border-2 border-teal/20 bg-teal/5 p-6 space-y-4">
            ...
            <p className="text-sm text-slate-600">
              Featured barber shops in {stateName} — verified listings with priority placement.
            </p>
      ...
      <section className="mt-8 rounded-2xl border-2 border-navy/20 bg-background p-6 space-y-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-navy border-b-2 border-teal/50 pb-1 inline-block">
              Barber Shops by City in {stateName}
            </h2>
            <p className="mt-1 max-w-2xl text-sm text-slate-600">
              Choose a city to browse barber shops and barbers in{" "}
              {stateName}, including haircuts, shaves, and beard services.
            </p>
          </div>
        ...
        {cities.length === 0 ? (
          <p className="text-sm text-slate-600">
            We don&apos;t have barber shops listed for {stateName} yet. As new data
